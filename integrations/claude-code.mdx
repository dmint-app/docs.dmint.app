...
```bash
claude mcp add dmint-gateway -- dmint-mcp run \
  --config /path/to/mcp_protection.json
```

<Note>
The policy file (`policy_file`), approval database (`approval_db`), and TTL (`approval_ttl`) are configured inside `mcp_protection.json`.
</Note>
